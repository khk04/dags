...
task4 = KubernetesPodOperator(
    ...
    env_vars=env_vars,
    image_pull_secrets=image_pull_secrets,
    cmds=["sh", "-c", "apt update && apt install -y curl && $BIOTOOLS_HOME/minimap2 -ax map-ont $COLO829/chm13v2.0.fa $OUTPUT/PAU61426_pass_4ddb6960_908efd09_0.fastq -t 1000 | $BIOTOOLS_HOME/samtools view -Sb - | $BIOTOOLS_HOME/samtools sort > $OUTPUT/minimap2_out.sorted.bam"],
    volume_mounts=[pvc_volume_mount, pvc_volume_mount_2, pvc_volume_mount_output],
    volumes=[pvc_volume, pvc_volume_2, pvc_volume_output],
    dag=dag,
)
